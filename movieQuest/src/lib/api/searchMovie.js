 ...
 */
export async function searchMovie(movieName, genreId) {
	let results = [];
	try {
		...
		if (movieName && genreId) {
			...
			url += `search/movie?api_key=${apiKey}&query=${movieName}`;
			const response = await fetch(url);
			const data = await response.json();
			results = data.results;
		} else if (genreId) {
			// If only genreId is present, use discover endpoint
			url += `discover/movie?api_key=${apiKey}&with_genres=${genreId}`;
			const response = await fetch(url);
			const data = await response.json();
			results = data.results;
		}
